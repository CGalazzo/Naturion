...
const HERO_ROWS = 8;
const HERO_WORLD_WIDTH = 2.42;
const HERO_WORLD_HEIGHT = 3.18;
const NPC_COLUMNS = 4;
const NPC_ROWS = 12;
...
const frameColumnForState = (state, elapsed) => {
  if (state === "running") return 7 + (Math.floor(elapsed * 12) % 4);
  if (state === "walking") return 3 + (Math.floor(elapsed * 8) % 4);
  return Math.floor(elapsed * 2.2) % 3;
};
...
const getPixelShadowTexture = () => {
  ...
  context.fillStyle = "rgba(5, 17, 16, .12)";
  context.fillRect(3, 6, 26, 5);
  context.fillStyle = "rgba(5, 17, 16, .24)";
...
export class DirectionalSpriteRig {
  constructor({ characterImage }) {
    ...
    this.texture = createSheetInstance(
      ...
      `overworld-animated-hero-${this.variant}`
    );
    ...
    this.sprite.name = `OverworldAnimatedHeroSprite-${this.variant}`;
    ...
    this.root.name = `OverworldAnimatedPlayer-${this.variant}`;
    this.root.add(this.sprite);
  }

  update({ state, velocity, elapsed }) {
    this.direction = directionFromVelocity(velocity, this.direction);
    const column = frameColumnForState(state, elapsed);
    ...
    const cadence = state === "running" ? 12 : state === "walking" ? 8 : 2.2;
    const step = Math.sin(elapsed * cadence);
    this.sprite.position.y = moving
      ? Math.round(Math.abs(step) * 3) / 96
      : Math.round(step * 1.2) / 96;
    this.sprite.scale.set(
      HERO_WORLD_WIDTH,
      HERO_WORLD_HEIGHT - (moving ? Math.round(Math.abs(step) * 1.2) / 96 : 0),
      1
    );
